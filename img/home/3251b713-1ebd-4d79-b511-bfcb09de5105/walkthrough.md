...
- **預期結果**:
    1. 活動錢包歸零。
    2. 儲值錢包 (Cash) 餘額增加 (增加金額為 `min(活動餘額, Cap)`)。
...
請至「遊戲管理 > 平台管理」或 `/admin/game-providers` 查看。

### [ ] AC 1：狀態變更即時性驗證
- **測試步驟**:
    1. 選擇任一「啟用」中的供應商，點擊操作按鈕編輯。
    2. 將狀態改為「停用」，填寫原因「測試停用」，點擊儲存。
    3. 確認列表中該供應商狀態從綠色「啟用」變為紅色「停用」。
    4. 驗證「最後操作人」和「更新時間」已更新。

### [ ] AC 2：維護模式顯示驗證
- **測試步驟**:
    1. 查看初始狀態為「維護中」的供應商（例如 Pragmatic Play）。
    2. 確認狀態顯示為橙色「維護中」標籤。
    3. 嘗試將其改回「啟用」，驗證更新是否成功。

### [ ] AC 3：排序邏輯驗證
- **測試步驟**:
    1. 查看列表，確認供應商按照「排序權重」由小到大排序。
    2. 選擇一個供應商，將排序權重改為 1，填寫原因後儲存。
    3. 刷新列表，確認該供應商移動到列表第一位。

### [ ] AC 4：操作日誌留痕
- **測試步驟**:
    1. 執行任一狀態變更操作（例如將 PG Soft 從「啟用」改為「維護中」）。
    2. 檢查瀏覽器控制台，確認有輸出 `[Provider Update]` 日誌記錄。
    3. 驗證日誌包含：供應商名稱、操作人、異動原因。

### [ ] AC 5：數據一致性驗證
- **測試步驟**:
    1. 查看平台管理列表中的供應商代碼（如 PG, EVO, JILI）。
    2. 前往「玩家管理 > 遊戲對局紀錄」(`/admin/game-logs`)。
    3. 確認遊戲對局紀錄中的「供應商」欄位值與平台管理中的代碼完全一致。

## OPE-302 遊戲列表管理 - 驗證指南

請至「遊戲管理 > 遊戲列表」或 `/admin/game-list` 查看。

### [ ] AC 1：複合搜尋準確性
- **測試步驟**:
    1. 選擇供應商「PG Soft」+ 狀態「已啟用」+ 名稱關鍵字「麻將」。
    2. 點擊查詢，確認僅顯示符合條件的遊戲（例如：麻將胡了2）。
    3. 清除篩選條件，測試單一條件搜尋是否正常。

### [ ] AC 2：最終生效比例邏輯驗證
- **測試步驟**:
    1. 查看「黃金帝國」(JILI)，確認當前為無標籤狀態，最終比例顯示 100%（類型基礎比例）。
    2. 點擊編輯，將行銷標籤改為「推薦 (120%)」，儲存。
    3. 確認最終比例從 100% 更新為 120%，且「標籤來源」變為「手動」。

### [ ] AC 3：標籤來源變更驗證
- **測試步驟**:
    1. 查看「招財貓」(PG)，確認標籤來源為「手動」。
    2. 點擊編輯，將標籤改為「無」，儲存。
    3. 確認標籤來源保持為「手動」（手動清除也算手動操作）。

### [ ] AC 4：快速切換開關
- **測試步驟**:
    1. 在列表找到「拳擊之王」(JILI)，確認狀態為「已停用」（開關OFF）。
    2. 點擊狀態開關，切換為「啟用」。
    3. 確認開關立即變為 ON，且列表刷新後狀態保持為 ACTIVE。
