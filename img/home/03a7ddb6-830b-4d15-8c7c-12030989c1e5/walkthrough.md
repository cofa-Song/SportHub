...
### 6. 玩家詳情增強 (玩家管理)
- **資訊透明化**: 在玩家詳情的基本資料區塊中，新增了「**註冊 IP**」與「**最後登入 IP**」欄位，協助進行風險控管。
- **檔案擴充**: 新增「**性別**、**生日**、**Email**」顯示欄位，並於編輯模式中支援針對上述欄位及「**VIP 等級**」、「**保級判定**」進行修改。
- **密碼管理**: 新增密碼重置功能，營運人員可於編輯視窗中直接幫玩家修改密碼（留空表示不修改）。
...
### 8. 代碼品質維護
- 清理了多個組件中的冗餘導入與未使用的 ref 變數（如 `MarketingTag`、`createFormRef` 等）。
- 修正了表格欄位對齊屬性的類型定義報錯。- **格式優化**: 金額欄位套用了 `$ ` 前綴與千分位格式化，提升數據可讀性。

### 6. 數據支撐 (Mock API)
- **類型擴充**: 更新了 `GameProvider` 實體定義。
